...
function ShapPanel({ result }: { result: ShapResult }) {
  ...
  return (
    <div className="mt-6">
      ...
      {/* Status */}
      <div
        className={`p-4 rounded-xl text-center text-white font-bold text-lg mb-4 ${
          isPain ? "bg-red-600" : "bg-green-600"
        }`}
      >
        {result.status}
      ...
      {/* SHAP explanation */}
      <div className="p-4 border border-gray-700 rounded-xl bg-gray-800 mb-4">
        ...
        <div className="flex gap-4 mt-3 text-xs text-gray-400">
          <span>
            <span className="text-red-400 font-bold">■</span> increases pain probability
          </span>
          <span>
            <span className="text-green-400 font-bold">■</span> decreases pain probability
          </span>
        </div>
      </div>
          ...
          <tbody>
            {result.shap_top3.map((row, i) => (
              <tr key={i} className="border-b border-gray-800 last:border-0">
                <td className="py-2 capitalize text-gray-200">
                  {row.feature}
                </td>

                <td
                  className="py-2 font-mono"
                  style={{ color: row.shap > 0 ? "#ef4444" : "#22c55e" }}
                >
                  {row.shap > 0 ? "+" : ""}
                  {row.shap.toFixed(4)}
                </td>
...
export default function PredictPanel() {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto mt-12 p-6 bg-gray-900 text-gray-200 rounded-2xl shadow-xl border border-gray-800">

      <h2 className="text-2xl font-bold text-center mb-6 text-gray-100">
        Manual Pain Prediction
      </h2>
        ...
        {Object.keys(form).map((key) => (
          <div key={key} className="flex flex-col">
            ...
            <input
              ...
              className="p-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        ))}
      </div>
      ...
      <button
        ...
        disabled={loading}
        className="w-full mt-6 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-500 transition shadow"
      >
